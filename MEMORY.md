...
2026-06-01 · OPENCODE ZEN NON-THINKING PIECE GENERATION · Opencode Zen art-piece chat-completions requests now send `thinking: { type: "disabled" }`, matching the DeepSeek non-thinking strategy for piece generation while leaving ordinary Zen text rewriting unchanged. This prevents Zen models such as `minimax-m3-free` from spending the response in reasoning/learning mode instead of final HTML/CSS/JS code blocks.
...
2026-06-01 · PIECE GENERATION OPTIMIZATION · Art-piece generation now uses provider-specific output budgets: `12000` for DeepSeek chat completions, `8192` for Anthropic/Google-style providers, and the safe `4096` gateway cap for other chat-completions providers such as Opencode Zen/Go. Piece-generation provider requests share the 20-minute route budget across up to 5 attempts. DeepSeek and Opencode Zen art-piece chat-completions requests send `thinking: { type: "disabled" }`; ordinary text rewriting keeps the normal provider request shape.
