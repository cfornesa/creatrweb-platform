...
2026-05-10 · INTERACTIVE PIECES · Art pieces no longer use JSON schemas for AI generation. System prompts have been refactored to provide code boilerplate and rely on Markdown blocks (`html`, `css`, `javascript`). Validated versions can now store explicit `html_code` and `css_code`. The Admin UI provides explicit code tabs and allows for manual code editing via the newly supported `POST /api/art-pieces/:id/versions` interface. AI retry loop feeds previous invalid attempts back into the repair prompt. Limits expanded to 5 attempts and 120s timeout.
    [Implemented 2026-05-10; verified from `art_piece_versions` table updates, `lib/art-pieces.ts` structure, `openapi.yaml`, and `admin-pieces.tsx` manual edit support.]
