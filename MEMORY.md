...
2026-05-08 · SYNDICATION · Blog URL scoping for WordPress.com and Blogger OAuth platforms is now implemented. `platform_oauth_apps.blog_url` stores the owner's blog URL. WordPress.com start route adds `blog=URL` to the authorize URL so the token is scoped to the correct blog; the callback reads `blog_id` directly from the token response. Blogger callback uses `GET /blogger/v3/blogs/byurl?url=URL` as the primary blog ID lookup, bypassing `users/self/blogs` which fails in Google Testing mode. The OAuth state store (`Map<string, { expiry: number; blogUrl? }>`) carries `blogUrl` from start to callback without a second DB round-trip. The `/admin/platforms` credentials dialog has a third "Your blog URL" input, and an "Update app settings" button lets the owner re-open the dialog even when OAuth is already connected.
    [Implemented 2026-05-08; verified from `platform-oauth-apps.ts` route, `platform-oauth.ts` state store + start/callback handlers, `openapi.yaml` + orval codegen, and `admin-platforms.tsx` dialog + platform card.]
