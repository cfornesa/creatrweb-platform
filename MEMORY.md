...
2026-05-06 · FEEDS · The site publishes feeds at extension-free primary URLs: `GET /atom` (Atom), `GET /jsonfeed` (JSON Feed 1.1), `GET /export/json` (mf2-JSON). Per-category: `GET /categories/:slug/atom` and `/categories/:slug/jsonfeed`. Per-page: `GET /p/:slug/atom` and `/p/:slug/jsonfeed`. The legacy extension-based routes (`/feed.xml`, `/feed.json`, `/export.json`, etc.) are kept as backward-compatible aliases — they work via direct port access or a proper Replit production deployment, but the Replit proxy intercepts them on `*.replit.dev` and on any custom domain CNAMEd to `*.replit.dev`.
    [Updated 2026-05-06; extension-free aliases added to bypass Replit proxy interception.]

2026-05-06 · FEEDS CATALOG · The feeds catalog (`GET /api/feeds`) always returns all category feeds — no `?category` param needed. Adding or deleting a category is reflected on the next request with no deploy. The `/feeds` page groups feeds into sections: "Site Feeds" for the three standard formats, one section per category (alphabetical), and optionally a per-page section via `?page=<slug>`. Feed URL generation is host-agnostic: origin is derived from `x-forwarded-proto`/`x-forwarded-host` (set by Replit's proxy for custom domains) or the raw Express `req.protocol`/`req.get("host")` for local. `PUBLIC_SITE_URL` is NOT used for feed URL generation; it is used only for AI HTTP-Referer headers and OG meta tags. Catalog paths now use the extension-free variants (e.g. `/atom`, not `/feed.xml`).
    [Updated 2026-05-06 to reflect extension-free primary URLs in feeds-catalog.ts.]
