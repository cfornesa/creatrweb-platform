...
- **Replit Webview Proxy Limitation**: The Replit dev proxy intercepts any request whose path ends in `.xml` or `.json` and serves `index.html` directly, bypassing Express entirely. This affects both the `*.replit.dev` webview URL and any custom domain CNAMEd to `*.replit.dev` (including `platform.creatrweb.com` while it points to the dev URL). **Fix**: the feeds catalog generates extension-free primary URLs (`/atom`, `/jsonfeed`, `/export/json`, `/categories/:slug/atom`, etc.) which the proxy does not intercept. The original extension-based routes (`/feed.xml`, `/feed.json`, …) are kept as backward-compatible aliases; they work on direct port access (`https://[repl]:5000/feed.xml`) and will work without workarounds once `platform.creatrweb.com` is configured as a custom domain on a Replit production deployment (where `router = "application"` forwards all paths directly to Express).
...
- **Outbound Feeds**: The site publishes Atom (`/atom`), JSON Feed (`/jsonfeed`), and Microformats2 export (`/export/json`). Per-category and per-page variants follow the same pattern (e.g. `/categories/:slug/atom`). The legacy extension-based URLs (`/feed.xml`, `/feed.json`, `/export.json`) are kept as aliases.
