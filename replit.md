...
- **AI vendor piece allowlist**: Piece generation (`POST /art-pieces/generate`) is restricted server-side to `google`, `mistral`, and `mistral-vibe`. Sending any other vendor returns a 422 with a descriptive message. The constant `PIECE_GENERATION_VENDORS` in `artifacts/api-server/src/lib/ai-settings.ts` is the single source of truth; a mirrored constant in `artifacts/microblog/src/hooks/use-owner-ai-vendors.ts` drives the frontend filtering.
