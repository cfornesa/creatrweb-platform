...
The header (`artifacts/microblog/src/components/layout/Navbar.tsx`) follows a strict left-to-right order: **logo → search bar → nav links → auth control / avatar → hamburger (only when something needs to spill)**. The hamburger is a *progressive overflow* — it contains only the items that didn't fit inline, never the items already visible. No nav link is ever rendered both in the inline strip and inside the open Sheet; the per-link `data-testid` pair `nav-link-<id>-inline` / `nav-link-<id>-sheet` is mutually exclusive at every viewport.

- **Measurement-based fit (desktop)**: a hidden off-screen measurer renders the full set of links + the search field + the auth button at their natural width. A `ResizeObserver` watches the container and recomputes `FitState = { authInline, searchInline, visibleLinkCount, hamburgerNeeded }` on every resize. The search width is subtracted from the budget *first* (search is mandatory on desktop), then nav links are fit greedily left-to-right by `sort_order`, then the "Log in / Register" button is fit if there's still room. The hamburger appears iff some links overflowed *or* the auth button didn't fit.
