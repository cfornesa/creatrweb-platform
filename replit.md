...
6. **Optional AI assistant**: if you want AI-assisted rewriting in the owner post composer and post edit flow, set `AI_SETTINGS_ENCRYPTION_KEY`, then open `/admin/ai`. AI is disabled per vendor by default. The owner can save one model slug and one encrypted API key for each supported vendor (`OpenRouter`, `OpenCode Zen`, `OpenCode Go`, and `Google`). Once one or more vendors are configured, the editor shows an AI vendor dropdown plus an `AI` button that sends the current draft to `/api/ai/process` using the selected vendor's saved model/key. Turning a vendor off hides it from the dropdown but preserves its saved configuration for later reuse.
