...
- **Replit Webview Proxy Limitation**: The Replit dev webview proxy intercepts requests for paths with file extensions (`.xml`, `.json`) and serves `index.html` directly, bypassing Express. Feed routes therefore return the React NotFound page on the default webview URL. Workaround: access feeds via the direct port URL (`https://[repl]:5000/feed.xml`) which bypasses the proxy. The production deployment (`platform.creatrweb.com`) is unaffected — `router = "application"` forwards all requests to Express.
- **Port Setup**: The Replit workflow sets `PORT=5000` inline (`PORT=5000 npm run dev`). `externalPort = 80` maps to `localPort = 5000` for the default webview URL. `externalPort = 5000` also maps to `localPort = 5000` for direct port access. `.env` has `PORT=5000` for local consistency.
