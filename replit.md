...
- **Immersive Viewer**: Local images and saved interactive pieces expose a lower-right **VR** affordance that opens a dedicated immersive route (`/immersive/images/:encodedRef`, `/immersive/pieces/:id`). Three.js pieces use viewer-managed OrbitControls with floor click-to-navigate and arrow key translation; p5 and c2 pieces use the browser-only Three.js gallery shell. Immersive routes are additive — existing post, page, and embed URLs are unchanged.
